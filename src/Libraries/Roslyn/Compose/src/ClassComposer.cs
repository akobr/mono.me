...
namespace _42.Roslyn.Compose
{
    ...
    {
        public ClassComposer(ClassDeclarationSyntax @class, IComposer parentComposer)
            : base(@class)
        {
            Composer = this;
        ...
        public ClassComposer(ClassDeclarationSyntax @class)
            : base(@class)
        {
            Composer = this;
        }
        ...
        public ClassComposer AddMethod(MethodOptions options)
        {
            ...
            var returnType = SyntaxFactory.ParseTypeName(options.ReturnType);
        ...
        public ClassComposer AddField(FieldOptions options)
        {
            if (!IsAtRoot())
            {
                throw new InvalidOperationException("A class must be selected (which is also a root to the composer) to add a field to it.");
            }

            var type = SyntaxFactory.ParseTypeName(options.FieldType);
