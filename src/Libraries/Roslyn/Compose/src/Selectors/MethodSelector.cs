...
using System.IO;
using System.Linq;
using _42.Roslyn.Compose.Attributes;
using Microsoft.CodeAnalysis;
using Microsoft.CodeAnalysis.CSharp.Syntax;

namespace _42.Roslyn.Compose.Selectors
{
    public class MethodSelector<T> : BaseSelector<T>
        where T : IComposer
    {

        public MethodSelector(StreamReader reader)
            : base(reader)
        {
        }

        public MethodSelector(MethodDeclarationSyntax node)
            : base(node)
        {
        }

        public MethodSelector(List<MethodDeclarationSyntax> nodes)
            : base(nodes.Cast<SyntaxNode>().ToList())
        {
        }
