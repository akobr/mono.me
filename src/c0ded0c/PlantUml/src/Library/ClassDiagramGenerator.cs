...
namespace c0ded0c.PlantUml.Library
{
    public class ClassDiagramGenerator : CSharpSyntaxWalker
    {
        ...
        public void Generate(SyntaxNode root)
        {
            if (_batchInProgress)
            {
                throw new InvalidOperationException("A generation batch is in progress; use GenerateInBatch method instead.");
            }
        ...
        private void GenerateInternal(SyntaxNode root)
        {
            Visit(root);
        ...
        public void GenerateInBatch(SyntaxNode root)
        {
            if (!_batchInProgress)
            {
                throw new InvalidOperationException("A generation batch is not in progress; use Generate insted or start a batch first.");
            }

            Visit(root);
        }

        public void FinishBatchGeneration()
        {
            if (!_batchInProgress)
            {
                return;
