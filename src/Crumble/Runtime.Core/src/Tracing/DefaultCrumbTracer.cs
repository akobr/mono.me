...
public class DefaultCrumbTracer(
    IJournalClient journal,
    ILogger<ICrumbTracer> logger) : ICrumbTracer
{
    ...
    public void LogBeforeMiddlewares(ICrumbInnerExecutionContext context)
    {
        try
        {
            journal.EnrollBeginningOfCrumb(context);
        }
        catch (Exception exception)
        {
            logger.LogError(
                exception,
                "Exception during making record in journal, the crumb {CrumbKey} with Id {Id} and context {ContextKey}",
                context.CrumbKey,
                context.ExecutionContext.Id,
                context.ExecutionContext.ContextKey);
        }
    }
    ...
    public void LogAfterMiddlewares(ICrumbInnerExecutionContext context)
    {
        try
        {
            journal.EnrollEndOfCrumb(context);
        }
        catch (Exception exception)
        {
            logger.LogError(
    ...
    public void LogException(Exception exception, ICrumbInnerExecutionContext context)
    {
        logger.LogError(
            exception,
            "Exception in crumb {CrumbKey} with Id {Id} and context {ContextKey}",
            context.CrumbKey,
            context.ExecutionContext.Id,
            context.ExecutionContext.ContextKey);
    }
