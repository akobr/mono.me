...
internal sealed class GrainSourceGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(allGrains, GenerateGrainKeyConstants);
    }
    ...
    private static CrumbGrainGenerateModel CrumbMethodTransform(GeneratorSyntaxContext c, CancellationToken t)
    {
        ...
        var crumbFullName = $"{classSymbol.ToDisplayString(FullyQualifiedWithoutGlobalFormat)}.{methodSymbol.Name}{methodNameSuffix}({methodArguments})";
        var crumbFullNameHash = MurmurHash3.Hash32(Encoding.UTF8.GetBytes(crumbFullName));
        var crumbFullNameHashString = $"{crumbFullNameHash:x8}";

        if (string.IsNullOrWhiteSpace(crumbKey))
        {
            crumbKey = $"{classSymbol.Name}.{methodSymbol.Name}.{crumbFullNameHashString}";
        }
        ...
        {
            CrumbKey = crumbKey,
            CrumbFullName = crumbFullName,
            CrumbFullNameHash = crumbFullNameHashString,
            CrumbName = crumbName,
            Method = methodSymbol,
            Class = classSymbol,
    ...
    private void GenerateGrainKeyConstants(SourceProductionContext context, ImmutableArray<CrumbGrainGenerateModel> all)
    {
        ...
        var simpleNamesSet = new HashSet<string>();
        ...
        source.WriteLine($$"""
                           ...
                           public static class CrumbKeys
                           {
                           """);

        foreach (var classGroup in all.GroupBy(g => g.Class.Name))
        {
            ...
            foreach (var grain in classGroup)
            {
                var crumbKey = grain.CrumbKey;
                var crumbKeyLastName = grain.Method.Name;

                if (simpleNamesSet.Contains(grain.CrumbName))
                {
                    crumbKeyLastName += $"_{grain.CrumbFullNameHash}";
                }

                simpleNamesSet.Add(grain.CrumbName);

                // language=c#
                source.WriteLine($$"""
                                           public const string {{crumbKeyLastName}} = "{{crumbKey}}";
                                   """);
            }
        ...
        context.AddSource($"GrainKeys.g.cs", code);
    }
    ...
    private void GenerateEntryPoint(SourceProductionContext context, ImmutableArray<CrumbGrainGenerateModel> all)
    {
        ...
        var firstGrain = all.First(); // TODO: [P1] this needs to be more robust
        var namespaceFull = firstGrain.NamespaceFullName;
        var namespaceHash = MurmurHash3.Hash32(Encoding.UTF8.GetBytes(namespaceFull));
